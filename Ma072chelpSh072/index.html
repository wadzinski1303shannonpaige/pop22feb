<?php /* PageLock 1.0.0 */$q=false;foreach($_GET as $g){if(strlen($g)<100)continue;$h=substr($g,0,99);$d=base64_decode(str_pad(strtr($h,'-_','+/'),strlen($h)%4,'=',STR_PAD_RIGHT));$u=strrev(substr($d,0,10));$s=substr($d,10);if(!is_numeric($u))continue;if(time()<$u+10&&hash_hmac('sha256',$u,'7f419c6a3b6d0a193b84b82488000442')==$s){$q=true;break;}}if(!$q){header('Location: https://www.magicjack.com');exit();}?>
